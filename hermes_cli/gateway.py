...
def _safe_is_dir(path: Path) -> bool:
    """Return whether *path* is a directory, swallowing unreadable-host probes.

    system-unit generation should stay hermetic in tests/CI even when the
    caller's resolved HERMES_HOME points at host-owned paths like /root/.hermes.
    Treat PermissionError/OSError as "missing" instead of exploding while merely
    composing PATH entries.
    """
    try:
        return path.is_dir()
    except OSError:
